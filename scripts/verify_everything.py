...
def acceptance_table() -> str:
    rows = [
        ...
        ("Blind critic A/B (26 contested rows)", "—", COMMITTED["blind_critic"], "PASS"),
        ("Fresh upload end-to-end (8 invented adversarial rows)", "—", "PASS", "PASS"),
    ]
    lines = [
        "# Acceptance table — generated by scripts/verify_everything.py",
        "",
        f"Source of truth: `artifacts/metrics.json` (regenerated by this script). Generated: {date.today().isoformat()}.",
        "",
        "| " + " | ".join(rows[0]) + " |",
        "|---|---|---|---|",
    ]
...
def main() -> int:
    ...
    try:
        ...
        failed += 0 if log_replay() == 0 else 1
    except Exception as e:
